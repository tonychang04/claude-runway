...
export class NativeManager{
 ...
 async usage(id){
  const p=this.profile(id);if(!p.auth?.loggedIn)throw Error('Finish signing in and confirm the account first.');
  const existing=this.state.sessions.find(s=>s.profileId===id&&s.kind==='usage'&&s.running);if(existing)return existing;
  return this.start(p,{cwd:this.configDir(id),kind:'usage'});
 }
 async start(p,{cwd,kind,model}){
  const id=crypto.randomUUID(),name='native-'+id.slice(0,8),dir=this.configDir(p.id);
  // Full native onboarding also covers theme and first-run setup; auth login alone
  // can report signed in while the first interactive session still needs setup.
  const args=kind==='login'?[]:kind==='usage'?['/usage','--tools','']:['--model',model,'--session-id',id,'--settings',JSON.stringify({statusLine:{type:'command',command:`${quote(process.execPath)} ${quote(path.join(import.meta.dirname,'native-statusline.mjs'))} ${quote(this.root)} ${quote(id)}`}})];
