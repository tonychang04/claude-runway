...
function render(s){
 ...
 for(const a of s.accounts){
  ...
  card.append(note,button('Reconnect account',()=>connectAccount(a),'text-button'));
...
$('#save-token').onclick=()=>act(async()=>{const f=new FormData($('#account-form'));await api('accounts',{name:f.get('name'),token:f.get('token'),replaceId});$('#account-form').reset();$('#account-dialog').close()});
...
async function pollLogin(){if(!loginId||loginPolling)return;loginPolling=true;try{const job=await api('login?id='+loginId);if(job.state==='complete'){loginId=null;$('#account-dialog').close();$('#account-form').reset();$('#login-flow').hidden=true;await load();return}$('#login-screen').textContent=job.screen;$('#login-link').hidden=!job.url;if(job.url)$('#login-link').href=job.url}catch(e){notice(e.message)}finally{loginPolling=false}}
